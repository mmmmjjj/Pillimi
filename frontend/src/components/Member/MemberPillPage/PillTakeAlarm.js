      ...
      if(element.type===false){
        ...
        result.push(<div className={`${style.checkAlarm2}`} key={element.date+element.img}>
          ...
          <div className="d-flex align-items-center d-row">
            <div className={`${style.imgsize2} ml-2`}><img src={element.img} ></img></div>
            <div className="flex-fill">
              <span>{element.time}</span><br></br>
              <span><PillList pills={element.pill}></PillList></span><br></br>
              <span>드실 시간입니다!</span><br></br>
            </div>          
          </div>
        </div>);
      } else {
