...
public class MedicineController {
    ...
    public ResponseEntity<BaseResponseBody> getTodayMedicine(@RequestParam(required = false) Long memberSeq) {
        ...
        if(!ObjectUtils.isEmpty(memberSeq)){
            ...
            familyService.checkFamily(member, protege).orElseThrow(() -> new NotFoundException(ErrorCode.THEY_NOT_FAMILY));
            member = protege;
        }

        return ResponseEntity.ok(BaseResponseBody.of(HttpStatus.OK, GET_MEDICINE_INFO, memberMedicineService.findTodayMedicineList(member)));
    }
}
