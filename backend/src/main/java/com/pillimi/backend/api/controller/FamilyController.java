...
public class FamilyController {
    ...
    public ResponseEntity<BaseResponseBody> requestFamily(@RequestBody FamilyRegistReq req){
        ...
        familyService.createFamily(member,req);
    ...
    @ApiResponses({
            @ApiResponse(code = 200, message = DELETE_FAMILY),
            @ApiResponse(code = 401, message = UNAUTHORIZED, response = ErrorResponse.class),
            @ApiResponse(code = 403, message = FORBIDDEN, response = ErrorResponse.class),
            @ApiResponse(code = 404, message = NOT_FOUND, response = ErrorResponse.class),
            @ApiResponse(code = 500, message = SERVER_ERROR, response = ErrorResponse.class),
    })
    public ResponseEntity<BaseResponseBody> deleteFamily(@RequestParam Long memberSeq){

        Member member = memberService.getMemberById(JwtUtil.getCurrentId()).orElseThrow(() -> new NotFoundException(ErrorCode.MEMBER_NOT_FOUND));
        Member target = memberService.getMemberById(memberSeq).orElseThrow(() -> new NotFoundException(ErrorCode.MEMBER_NOT_FOUND));

        familyService.deleteFamily(member,target);
        return ResponseEntity.ok(BaseResponseBody.of(HttpStatus.OK, DELETE_FAMILY));
    }
